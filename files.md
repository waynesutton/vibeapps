...
- `src/components/admin/AIJudgeResults.tsx`: Admin AI Judge results view with Results, Stats, Convex Recap, and Hackathon Report tabs: Run AI Review with live progress counts, ranked per-criterion scores and reasoning, verified Convex facts, used vs installed components, source and URL liveness badges, frontend hosting, hackathon event and discrepancy indicators, inline score editing, retries, and per-submission privacy-safe Copy brief actions. Stats summarize Convex usage and score distribution. The gated Convex Recap generates a no-PII markdown handoff for the Convex team from saved results, while the existing Hackathon Report generates the organizer report with rankings, links, teams, and participation
