...
- `package.json`: Project dependencies and scripts (`dev`, `dev:backend` for `convex dev`, `build`, `lint`, `lint:fix`, `typecheck` for the app via `tsc -b`, `typecheck:convex` for the backend)
- `.env.example`: Documents every env var the code reads. Frontend `VITE_*` keys to copy into `.env.local`, the Convex deployment env set with `npx convex env set` (Clerk keys, `FIRECRAWL_API_KEY` and `CONTEXT_DEV_API_KEY` which `convex.config.ts` requires, Resend, GitHub, Luma), and the Clerk `convex` JWT template with the claims `ensureUser` reads
...
- `.agents/skills/`: Shared agent skills (Convex quickstart, auth setup, component creation, migration helper, performance audit, plus design and workflow skills) for consistent agent behavior. The `convex-*` skills are generated by `npx convex ai-files install` and refreshed by rerunning it
...
- `prds/social-proof-judging.md`: one shared social proof snapshot per submission link for the AI judge and human judges (Firecrawl X engine with oEmbed fallback, Bluesky public API, LinkedIn recorded as unverified), the `social-proof` preset criterion, the judge page card, the admin refresh button, and the live verification log
- `prds/fable-judge-jev-second-opinion.md`: run time chat model selection via the `AI_JUDGE_MODEL` env with Claude Fable 5 as the fallback, a per group Jev second opinion that scores the rubric with calibrated probabilities and never changes Fable's scores, and a site wide Jev spam classifier whose confidence is the probability of the chosen verdict; includes the live gateway smoke test results
- `prds/ai-judge-sponsor-stack-detection.md`: fixes `@agentmail/convex` component name resolution (map entry plus `@scope/convex` fallback) and adds recorded-only sponsor stack detection (AgentMail, Firecrawl, OpenAI via component, SDK, API key, HTTP, or gateway) and model provider detection to the AI judge, stored on results, shown in the prompt, admin and public UI, exports, and stats
- `prds/ai-judge-human-criteria-screenshot.md`: AI judge mirrors the group's human judging criteria into its rubric (per group toggle, `human-<criteriaId>` keys, AI-only weights, prune on delete), attaches a Firecrawl screenshot of the live app to the model with a text-only retry, fixes the legacy modal wiping custom weights, and updates the admin docs (Convex AI gateway, AI judge vs agent judges)
...
- `convex/_generated/`: Auto-generated Convex files (API definitions, data model types)
  ...
  - `ai/guidelines.md` & `ai/ai-files.state.json`: Convex rules for coding agents, written by `npx convex ai-files install`. `AGENTS.md` and `CLAUDE.md` tell agents to read the guidelines first
- `convex/schema.ts`: Database schema definition with all tables and indexes (includes `lumaEvents` plus `settings.sidebarWidgets` / `lumaConfig`, `judgingGroups.hideLumaEvents`, `submissionJudgeCompletions` table for multi-judge OCC-safe completion tracking, `aiJudgeResults` table for AI Judge scores, reasoning, run status, detected `frontendHosting`, `authProvider`, `usesAiGateway`, and `aiModelIdsDetected` per group+story, `aiFrontendWeights` per-platform frontend checker weights and persisted AI group summary metadata on `judgingGroups`, `videoTranscripts` table for scraped video demo transcripts used by the AI judge, `socialProofSnapshots` table (one row per story per `linkedinUrl`/`twitterUrl` field with platform, post vs profile kind, status, engagement counts, and `fetchedAt`; indexes `by_story` and `by_story_field`) shared by the AI judge and the judge page card, `aiJudgeResults.sourcesUsed.socialProof`, `stories.hackathonLog` for pasted hackathon.md content (capped and secret-redacted), `aiJudgeResults.logDiscrepancies` cross-check notes and `hackathonLogEvent` parsed from the repo or pasted hackathon.md header, and deprecated hackathon skill API fields on `judgingGroups` plus the deprecated `hackathonRegistrations` table kept only so existing rows stay valid)
...
- `convex/judgingCriteria.ts`: Judging criteria and scoring questions management with 1-10 star ratings; the delete guard uses the `judgeScores.by_criteriaId` index, and `pruneAiSettingsForDeletedCriteria` removes a deleted criterion's `human-<id>` AI weight and off flag from the group on single delete and bulk save
...
- `convex/aiJudge.ts`: AI Judge (Best Use of Convex) backend: built-in rubric (five Convex criteria plus a low-weight "Live app status" liveness criterion) extendable per group with custom criteria, the group's mirrored human judging criteria (`aiIncludeHumanCriteria` toggle via `updateAiIncludeHumanCriteria`; `humanCriteriaToRubric` maps each `judgingCriteria` row to a `human-<criteriaId>` key with the question as label and the description plus human-scale note as description, read live so edits apply on the next run), and per-criterion on/off toggles via `getRubricForGroup(group, humanCriteria)` (filters `aiDisabledCriteria`, never returns an empty rubric), `updateAiCustomCriteria` (up to 10 admin-defined criteria with key/label/description validation, the `human-` prefix reserved, and stale weight/disabled-flag pruning that keeps mirrored human keys), `updateAiSystemPrompt`/`getAiPromptConfig` for the editable system prompt with `DEFAULT_AI_JUDGE_PROMPT_BODY` and reset-to-default (rubric rows carry `source: builtin | custom | human`), `updateAiRubricWeights` accepting weights plus disabled keys for built-in, custom, and mirrored human keys (at least one criterion must stay enabled) and per-platform `frontendWeights` for the frontend checker (fixed keys `codex-sites`/`convex-hosting`/`vercel`/`netlify`/`other`, default 1, all-default clears storage; the detected platform's weight multiplies the `frontend-checker` criterion weight in `computeWeightedScore` at read time), `startReview`/`retrySubmission` admin mutations that queue submissions and schedule sequential analysis, `updateResultScore` for admin edits with recomputed totals, `getGroupAiResults` admin query with progress counts plus optional persisted `groupSummary` (markdown, model, fingerprint, `isStale`) for the cohort recap, `getGroupAiReportData` admin-only query for the hackathon report (submissions with team names/members and AI results; team emails never exposed publicly), and public results queries with the same password/public/admin-bypass gate pattern as human results (`getPublicAiResultsInfo`, `validateAiResultsPassword`, `verifyAiResultsPassword`, `getPublicAiResults`, `getValidatedAiResults`); never touches `submissionStatuses`, `judgeScores`, or alerts
- `convex/aiJudgeAnalysis.ts`: `analyzeSubmission` internal action for the AI Judge plus public `generateGroupSummary` for an optional cohort recap: fetches GitHub repo context (metadata, file tree, `convex/` files, README, root hackathon log files like hackathon.md/changelog.md/task.md/files.md capped at 5k chars each, and `.agents/skills/` skill paths via optional `GITHUB_TOKEN`), fetches an optional `/hackathon.json` manifest from the live app origin for self-reported context, scrapes the live URL with Firecrawl (optional `FIRECRAWL_API_KEY`; the same request captures a screenshot of the rendered page that is attached to the model as an image on the first attempt only, the retry is text only, and `sourcesUsed.screenshot` records use), runs a deterministic liveness check of the live app URL (GET with 15s timeout; result stored as `urlCheck` and the liveness score is server-side capped at 2 when the URL is dead/404), fetches social proof snapshots for the submission's LinkedIn and X or Bluesky links through `fetchSocialProofContext` (rendered as a `SOCIAL PROOF (verified by direct fetch)` prompt section, with a fixed rule against inventing engagement and `sourcesUsed.socialProof` recording whether a readable post was included), detects the frontend hosting platform deterministically via `detectFrontendHosting` (URL host suffixes, `x-vercel-id`/`x-nf-request-id`/`server` response headers, and repo signals like `.openai/hosting.json`, `@convex-dev/self-static-hosting`, `vercel.json`, `netlify.toml`; stored as `frontendHosting` with evidence, surfaced to the model as a FRONTEND HOSTING CHECK section, and the frontend-checker score is capped at 3 when the live URL is dead), detects Convex components deterministically from the repo's `package.json` (`@convex-dev/*` deps) and `convex.config.ts` imports (stored as `componentsDetected` and rewarded with higher advanced scores in the prompt), builds the system prompt from the group's custom prompt (with `{{rubric}}` substitution) or the default, always appends the JSON response contract, fetches the submission's video demo transcript via `fetchVideoContext` (included in the prompt as an unverified builder narrative section that never overrides verified facts and never penalizes a missing video), prompts an LLM with the group's effective rubric (built-in, custom, and mirrored human criteria; fixed non-editable prompt rules say `human-` criteria are judged from the same evidence a human would use and a screenshot informs UI judgments only) through the shared Convex AI gateway caller in `convex/lib/llm.ts`, parses structured JSON scores/reasoning against that rubric, and saves results (including `sourcesUsed.videoTranscript`) or per-submission failures for retry; supports the single-file hackathon skill by injecting a pasted `stories.hackathonLog` into the PROJECT LOG FILES prompt section when the repo returned no hackathon.md (the repo copy always wins), and computes never-scored `logDiscrepancies` by comparing the parsed hackathon.md header against detected facts (frontend platform vs `detectFrontendHosting`, claimed components vs `convex.config.ts` scan, claimed auth vs the detected provider, claimed AI models vs model ids found in `convex/` source); detects auth via `detectAuthProvider` (dependency signal from the root plus monorepo workspace `package.json` files, with file-signal fallbacks: `convex/auth.config.ts|js` naming a clerk./workos domain proves that provider, and `convex/auth.ts` plus `@convex-dev/auth` in any manifest proves Convex Auth) and AI model evidence via `detectAiModelEvidence` (`convexGateway("provider/model")` literals set `usesAiGateway` plus the model id, and `model: "..."` literals passed to OpenAI/Anthropic SDK calls are captured for apps that skip the gateway; it also names `modelProvidersDetected` from provider SDK deps, API key env vars, and model id families) and sponsor integrations via `detectSponsorStack` (AgentMail, Firecrawl, OpenAI, each tagged component, sdk, api_key, http, or gateway with the matched signals; the `SPONSOR_DEFS` table and a fixed prompt rule keep these recorded-only facts out of every score), all fed to the judge prompt as code facts (AI MODEL EVIDENCE and SPONSOR STACK EVIDENCE sections) and stored on the result row as `authProvider`, `usesAiGateway`, `aiModelIdsDetected`, `modelProvidersDetected`, and `sponsorStack` so Stats, Recap, and the Hackathon Report can roll them up without a hackathon.md. `COMMUNITY_COMPONENT_PACKAGES` maps non `@convex-dev` component packages (Firecrawl, AgentMail, Exa, Context.dev, Browser Use, agent-ready) to the `components.<name>` property teams reference, and `nameFromConfigImport` resolves `@scope/convex` imports to the scope. Also fetches published `/hackathon.md` from the live origin as a third log fallback, maps known community Convex components from every workspace manifest, and records live-site Convex signals when the repo is unreachable. `generateGroupSummary` uses the same gateway caller on saved `aiJudgeResults` only, stores markdown on `judgingGroups`, and does not rescan repos or change scores
- `convex/spamCheck.ts`: AI spam check backend: `listSpamResults` admin query (story-enriched scan rows with server-side verdict filter, date range, sort, and live counts), `startBatchScan` (queues up to 100 submissions into the dedicated `spamWorkpool`, skip-or-rescan aware, optional date range via the built-in `by_creation_time` index), `scanStory` single re-scan, `listMarkedSpam` (every story with isSpam true straight from the stories `by_isSpam` index for the marked-spam review section, enriched with author, marking admin or agent, and reason, with an optional marked-date range), `markAsSpam`/`unmarkSpam` (hide + label + in-app alert + reason email, fully reversible), `bulkMarkAsSpam`/`bulkHide`/`bulkDelete` (delete cascades comments, votes, ratings, bookmarks, scan rows, and stored images), the admin-editable AI system prompt (`DEFAULT_SPAM_SYSTEM_PROMPT`, `getSpamPrompt`/`setSpamPrompt` with reset-to-default, stored under appSettings key `spamCheckSystemPrompt`; `getSpamAnalysisConfigInternal` hands the analysis action the prompt plus the `jevClassifierEnabled` flag in one call), and internal plumbing (`autoScanStory` scheduled from submit mutations and gated by the auto-scan toggle, `markRunning`, `getStoryForAnalysis` with duplicate-URL count, `saveResult` which also runs the agent auto-mark when enabled), plus spam automation settings (`getSpamAutomation`/`setSpamAutomation`: auto-scan toggle, agent auto-mark with confidence threshold, notify-on-auto-mark, and the Jev classifier toggle under `spamJevClassifierEnabled`, all stored in appSettings and enforced server side with activity logging) and the submitter dispute loop (`requestSpamReview` owner mutation stamping `spamReviewRequestedAt` and logging `spam.reviewRequested`, `getMySpamStatus` owner-scoped status query for the notifications button, `dismissSpamReviewRequest` admin mutation that clears a request while keeping the mark)
- `convex/spamCheckAnalysis.ts`: `analyzeSubmission` internal action for spam scans: deterministic checks in parallel (live URL GET with timeout, GitHub repo reachability and file count with empty-repo detection via optional `GITHUB_TOKEN`, video/LinkedIn/X link liveness), Firecrawl component scrape of the app URL, then an AI verdict (spam/suspicious/clean with confidence and reasons). When the admin `jevClassifierEnabled` setting is on it calls `classifySpam` first (Jev choice question plus boolean flags, confidence is the probability of the chosen verdict, provider recorded as `typesafe`, probabilities summarized in the reasoning); otherwise or on Jev failure it uses the chat model through `callLlm`, and a deterministic heuristic when the gateway call fails (result records provider "heuristic"); failures are recorded on the result row and never thrown upstream
- `convex/lib/llm.ts`: Shared gateway callers for the AI judge and the spam check, two doors into the hosted Convex AI gateway (deployment service token, no provider API keys). Chat door: `callLlm` sends the system prompt and user message (plus an optional `imageUrl`, attached as an `ai` SDK file part with `mediaType: "image"` for the live app screenshot) through `generateText` to the model from `resolveLlmModel()`, which reads the typed `AI_JUDGE_MODEL` env and falls back to `FALLBACK_LLM_MODEL` (`anthropic/claude-fable-5`); `providerOf` keeps the id prefix as the provider on result rows. Decisions door: `evaluateRubric(state, rubric)` and `classifySpam(instructions, state)` call `JEV_MODEL` (`typesafe/jev-1.13`) through `convexGateway.evaluationModel` and the `ai` SDK's `experimental_evaluate`; the rubric helper asks one 5 level score question per criterion (`rubricLevels`), maps Jev's fractional level to 1 to 10, and reports `peakProbability` as confidence, while the spam helper asks a choice question (spam, suspicious, clean) plus the `SPAM_FLAGS` booleans, returns the probability of the chosen verdict as confidence, and turns flags above `SPAM_FLAG_THRESHOLD` into reasons
...
- `convex/socialProof.ts`: Social proof snapshots for judging: `fetchSocialProofContext` action helper (24h per URL cache in `socialProofSnapshots`, one row per story per link field, never throws) routes each classified link to Firecrawl's `x-twitter` engine (`parseXMarkdown` undoes markdown escapes and reads `Likes: N | Retweets: N`, `Author:`, `Posted:`, and the `## Post` section) with a text only `publish.twitter.com/oembed` fallback, Bluesky's public AppView (`resolveHandle` then `getPostThread` for like, repost, reply counts), a LinkedIn HEAD check recorded as unverified because LinkedIn returns 200 for nonexistent posts, and `profile_only` for account URLs; `formatSocialProofForPrompt` renders the `SOCIAL PROOF` prompt section; `getSocialProofForStory` public query (judge session or `judging.ai`, story must belong to the group) feeds the judge page card; `refreshSocialProofForGroup` admin mutation schedules `refreshSocialProofForStory` per submission with links to re snapshot every team at once
- `convex/lib/socialLinks.ts`: Pure `classifySocialUrl` for X (`x.com`, `twitter.com`), Bluesky (`bsky.app`), and LinkedIn URLs returning platform, post vs profile kind, a normalized URL, post id, and handle; `socialPlatformLabel` for shared copy. Mirrored key constant lives in `groupSection.tsx`
...
- `convex/lib/reactionEmojis.ts`: The `ALLOWED_EMOJIS` list and `ReactionEmoji` type for DM reactions. Imports nothing so the frontend can render a picker from it; `convex/dmReactions.ts` builds the matching validator and is type-checked against it so the two cannot drift
...
- `convex/convex.config.ts`: Convex app definition installing the Resend, crons, workpool (plus a separate spamWorkpool), Agent Ready, rate limiter, Firecrawl (bound to `FIRECRAWL_API_KEY`), and Context.dev (bound to `CONTEXT_DEV_API_KEY`, used for video transcript scraping) components, and declaring the typed app env (`FIRECRAWL_API_KEY`, `CONTEXT_DEV_API_KEY`, optional `AI_JUDGE_MODEL` for the judge's chat model)
...
- `src/lib/countdown.ts`: Pure helpers for the submission deadline countdown: `splitRemaining` (days, hours, minutes, seconds, never negative, `expired` flag), `formatDeadline` (viewer's local time with short zone name via `Intl.DateTimeFormat`, year only when it differs from the current one), `describeRemaining` (screen reader summary), and the `CountdownSize` type. No React import so admin screens and the component share them
...
- `src/components/CountdownTimer.tsx`: Live countdown to an absolute epoch ms deadline rendered in each visitor's local time. `large` shows stacked days, hours, minutes, seconds with colon separators and the deadline line; `compact` is a one-line bar with a clock icon. Ticks once a second through a `useNow` interval hook, `role="timer"` with a full aria-label summary while the digits are aria-hidden, `tabular-nums` so widths do not jitter, shows "Deadline passed" after the deadline, display-only (never gates submissions)
...
- `src/components/judging/SocialProofCard.tsx`: Judge page card under Project Links that reads `api.socialProof.getSocialProofForStory` with the judge session and renders one row per snapshot: platform icon, Post or Profile badge, Not found badge when the post is gone, author and date, clamped post text, like, repost, and reply pills, an honest status note (LinkedIn unverified, profile only, unreadable), the capture time, and an Open link; renders nothing until a snapshot exists so the plain links above are unchanged
...
- `src/components/admin/judging/groupSection.tsx`: Shared utilities for group workspace sections: `GroupDetails` type, submission field/section/visibility definitions, custom question types and key generation (`makeQuestionKey`, `mergeVisibility`), AI rubric definitions, date and datetime-local input helpers (`tsToDateTimeInput`, `dateTimeInputToTs` convert in the admin's local zone), and UI primitives (`SectionCard` with optional `headerAction` slot, `SaveFooter`, `HeaderSaveButton`, `UrlRow`, `TogglePill`, `useSaveState`), and the `HUMAN_CRITERION_PREFIX` / `humanCriterionKey` mirror of `convex/aiJudge.ts`
...
- `src/components/admin/EditJudgingGroupModal.tsx`: Legacy comprehensive editing modal, superseded by the group workspace sections above; no longer referenced but kept in the repo. Its rubric weight save preserves stored custom and human criteria weights it does not edit
- `src/components/admin/AIJudgeResults.tsx`: Admin AI Judge results view with Results, Stats, Convex Recap, and Hackathon Report tabs: Run AI Review with live progress counts, ranked per-criterion scores and reasoning (with a `SecondOpinionScore` chip showing Jev's 1 to 10 score and confidence next to each Fable score when the group's second opinion toggle is on, flagged when the two differ by 2.5 or more, plus a truncation hint), verified Convex facts, used vs installed components, auth provider and AI Gateway badges, source and URL liveness badges, frontend hosting, hackathon event and discrepancy indicators, inline score editing, retries, and closed per-submission brief panels with copy and `.md` save actions. Stats summarize Convex usage, auth providers, AI Gateway, components, and score distribution. The gated Convex Recap generates a no-PII markdown handoff for every completed submission, supports copy and `.md` export, and can include an optional persisted AI cohort summary with stale-result detection. The existing Hackathon Report keeps the organizer report with rankings, links, teams, participation, auth, and AI Gateway
...
- `src/types/index.ts`: TypeScript type definitions for the frontend, built on `Doc` and `Id` from `convex/_generated/dataModel` (path is `../../convex/...`; the old `../convex/...` pointed at a folder that does not exist and made these types `any`)
