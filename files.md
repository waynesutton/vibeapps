...
- `prds/submission-server-error-fix.md`: Root causes of the redacted "Server Error" on the event submit page and the story edit form (plain `Error` redaction on prod, duplicate event URL, public pages strip team member emails so `updateOwnStory` rejected every hackathon edit, edit form tag limits out of sync with settings, Convex auth timing). All edits applied, including the `stories.ts` and `users.ts` ConvexError and team merge changes (hook paused for one session); verified on dev, prod deploy pending
...
- `convex/users.ts`: User management functions (queries, mutations, admin functions, mention search, recent vibers discovery, emoji theme preferences). `getMyUserDocument` returns a validator-safe subset of the current user row so extra schema fields cannot crash signed-in pages. `getAuthenticatedUserId` and `ensureUserNotBanned` throw `ConvexError` with plain language messages ("Please sign in and try again.", "Your account is still syncing...") that survive prod redaction.
...
- `convex/stories.ts`: App submission functions (create, update, approve, search) with multi-image support, a per-group duplicate project URL guard on judging group submissions, pasted `hackathonLog` handling (20k cap, secret redaction), generic admin-added form field persistence via `dynamicFieldValues`, and public list/detail queries that strip submitter email, team emails, and moderation fields. User facing checks throw `ConvexError` so prod shows the real message; `updateOwnStory` and `updateStoryAdmin` accept `teamMembers[].email` as optional and merge by position so a member sent without an email keeps the stored one
...
- `convex/hackathon.ts`: Shared hackathon URL helpers `normalizeProjectUrl` and `groupHasDuplicateUrl` (per-group duplicate URL guard used by `stories.submit`; hidden or rejected entries free the URL), plus the public `isProjectUrlTakenInGroup` query (`{ groupId, url }` to boolean) the event submit form uses to warn about a duplicate URL before the mutation rejects it; the old hackathon skill API endpoints were removed in favor of the single-file hackathon.md flow
- `convex/hackathonLog.ts`: Single-file hackathon skill utilities: `HACKATHON_LOG_MAX_CHARS` 20k cap (over the cap throws `ConvexError` so prod shows the real message), `redactSecrets` (replaces sk-/pk*/ghp*/github_pat_/xox/AKIA/JWT/Convex deploy key shapes with `[redacted]`), `sanitizeHackathonLog` used by all four submission paths, and `parseHackathonLogHeader` which deterministically extracts event/project/whatItDoes/liveApp/repo/frontend/convexDeployment/components/features (accepts both "Features" and "Convex features" labels)/auth/aiModels/started/lastUpdated from "- **Field:** value" markdown lines without ever throwing on malformed input
...
- `src/lib/submissionDownloads.ts`: Pure builders for judging group submission downloads, typed off the `exportGroupSubmissions` return type. `buildSubmissionCsv` (formula-safe cells, CRLF, UTF-8 BOM), `buildSubmissionJson` (group metadata plus rows), `buildSubmissionMarkdownFiles` (one file per submission with JSON-quoted YAML frontmatter, inline-escaped short fields, and the long description written as the author's raw Markdown), `buildSubmissionMarkdownIndex` (the ZIP `README.md`), and `buildSubmissionMarkdownZip` which lazy loads JSZip. Also exports `escapeCsv`, `escapeInline`, and `safeFilename`
...
- `src/components/admin/judging/GroupSubmissionsSection.tsx`: Search-and-add any site submission to the group, auto-include tags/date range/match rule config, and sync matching submissions action (downloads moved to the View submissions toolbar)
- `src/components/admin/judging/GroupSubmissionsTableSection.tsx`: Read-only roster table for one judging group (View submissions section) built on TanStack Table v9 with sorting and pagination features; columns are submission title plus slug, submitted date, tag pills with a +N overflow, submitter name and profile link, and LinkedIn, GitHub, X, and site links, each with a copy-to-clipboard button and a new-tab link; one search box filters title, slug, submitter, and tag names, page size is 25/50/100, a Download dropdown (`SubmissionDownloadControl`, shown only to `judging.results` holders) sits at the end of the toolbar, and no mutation is wired into the section
- `src/components/admin/judging/SubmissionDownloadControl.tsx`: Download dropdown for the View submissions toolbar. Fetches `exportGroupSubmissions` on demand, builds CSV, JSON, or a Markdown ZIP client side with the `src/lib/submissionDownloads.ts` builders, triggers the browser download, and reports success or failure through `sonner` toasts with `aria-busy` on the trigger while preparing
...
- `src/lib/convexErrors.ts`: `getConvexErrorMessage(error, fallback)` turns anything thrown by a Convex mutation into text a user can act on: prefers `ConvexError.data` (string or `{ message }`), strips the `[CONVEX M(...)] [Request ID: ...]` prefixes and the `Called by client` trailer, keeps the first meaningful line of a dev stack trace, and maps a redacted prod `Server Error` to the fallback plus the request id so support can find the log. Used by the story submit form, the event submit page, and the story edit form
