...
- **How to judge guide, second pass.** The public `/judging/{slug}/howtojudge` page and its Markdown export gain two sections. **Shortlist round** (shortlist mode only) covers how the cut was made (AI top N plus organizer picks, or hand picked when the AI judge is off), what counts toward progress, and, when below-cut rows are visible, the Shortlist / Below the cut / All filter, the "N below the cut, read only" pill, the lock and "Not in this judging round" banner, why score buttons are missing, asking for a second look in notes, and the "Nothing shortlisted yet" state. When below-cut rows are hidden it says so and how to ask for a missing app. **Reading scores** covers your own scores (save on click, Edit Scores, only completed submissions count, human and AI kept apart), the `AI #12/40 7.2/10` badge and AI review card (rank by weighted total, AI always out of 10 vs the group scale, Hide AI scores to judge blind), the results page (the four stat cards, average vs total, judges completed line, per judge breakdown, Criteria Performance), and the AI results page (rank order, chips for sources and the live link check, what expanding a row shows); the results blocks appear only when those links are shareable. TL;DR gains a shortlist step, navigation tips add the arrows and # box to the export plus the round filter and AI toggle, and troubleshooting adds rows for unscorable below-cut submissions, an empty shortlist, a missing app, and a hidden AI badge. New shared helpers `shortlistBlocks`, `readingScoresBlocks`, `navigationTips`, `troubleshootingRows` so the page and export stay in sync. Frontend only. PRD: `prds/how-to-judge-guide-pass.md` (2026-09-23).
- **Content policy notice.** A new Content policy section in admin Settings sets one notice that shows in the About modal and directly above the submit button on `/submit`, judging group submit pages (`/judging/{slug}/submit`), and custom forms (`/submit/{slug}`). Admins can turn it on or off, edit the text (plain text, line breaks kept, 1000 characters max with a live counter and Reset to default), add an optional "Read the full policy" link, and see a live preview. It ships on with a default: "Share what you built. Keep it clean. Admins and moderators can hide or remove any app, comment, or profile at any time. NSFW, adult, hateful, or illegal content is not allowed. Neither is spam, malware, or anything that collects user data without consent." `settings.update` trims and caps the text and only accepts `http://` or `https://` links (`ConvexError`, so the message survives prod redaction). The live `/llms.txt` and `/vibeapps.md` gain a Content policy section from the same setting, so agents read the same rule. The About modal now scrolls when it is taller than the screen. PRD: `prds/content-policy-notice.md` (2026-09-23).
- **AI results status filter and failure reasons.** The Pending, Reviewing, Completed, and Failed cards on admin AI results are now toggles that filter the list. A Status select (with counts) and a Sort select (AI rank, Title A to Z, Failure reason) sit next to Eligibility, with a "Showing N of M" line and Clear filters. With Failed selected, a Why they failed panel groups failed rows by error message (first line, URLs and long numbers collapsed so repeats group together) with counts; pick a reason to see only those rows, then Retry each one. Rank numbers stay fixed while filtering or sorting, and long error text wraps in the row. Frontend only. PRD: `prds/ai-results-status-filter.md` (2026-09-23).
- **Judging results UI polish.** Admin AI results rows are rebuilt: rank, title, and review state up top with the score pinned top right; the sources line (repo, site, video, social) and recorded fact chips wrap under the title instead of squeezing into multi line blobs past the card edge; a hairline toolbar at the bottom holds "Reviewed via" plus Shortlist, Brief, and a labeled Details toggle, all one height. Fact chips come from a new shared `FactChip` (neutral, good, warn, bad, info) with dark theme tints, nowrap text, and truncation with the full text in the tooltip. The duplicate Shortlisted badge next to the title is gone since the toolbar toggle shows the state. The shortlist bar and the score editor wrap on narrow widths. The shared `Button` is now `inline-flex`, so icons sit beside labels ("Re-run AI Review", "Shortlist top 10", "Copy Markdown" used to stack the icon above the text), `size="sm"` is really small (`h-8`, `text-xs`; the old regex never matched so it rendered at default size), and it has a visible keyboard focus ring. Results dashboard: criteria bars now scale to the group's score range instead of a hardcoded 5 and never pass full, CSV export escapes quotes inside titles and comments, selected judge tabs flip to the filled style, and the purple Agent badge is neutral. Judge tracking: purple accents moved to tokens and the selected judge tab matches. PRD: `prds/judging-results-ui-polish.md` (2026-09-23).
- **Event window with time, and late submission labels.** The judging group event window (AI judge section) now takes a date and time instead of a date. Start still feeds the build timeline check; end is the submission deadline. Each field shows a "Saves as Sun, Aug 9, 11:59 PM PDT" line in the admin's zone, end must be after start, and untouched fields are not resent so a stored deadline is never moved by an unrelated save. Every submission gets a read time `submissionTiming` fact (`on_time`, `late`, or `no_deadline_set`) computed from the story's submit time against the group's current end; nothing is stored, so changing the deadline relabels every row live with no backfill. The AI judge receives a `SUBMISSION TIMING` facts section (submitted at, deadline, how late) and a fixed rule to score every criterion normally, never move a score for timing, and open its overall reasoning with a late note; the server appends the note if the model forgets. Humans see a red **Late submission** chip with a tooltip (submitted at, how late, the deadline, eligibility is the organizer's call) in the judging interface search list and title row, the admin View submissions Submitted column, admin AI results (plus **Submitted on time** and **Late** options in the Eligibility filter, and a Late line in the brief and hackathon report), and the public AI results page. The judge login page shows the window with time and says Ends or Ended. The How to judge page and its Markdown export gain a Late submissions section (deadline in the reader's zone, what the label means, score normally and note it) when the group has an end; the AI review section says the AI knows the deadline. Admin docs: Event dates rewritten to separate the auto include window from the event window, a new Event window and late submissions subsection under AI judge, the Eligibility filter, the chip in Judge flow, and the How to judge page section. The label is deliberately "Late submission", not "Disqualified": the platform flags, organizers decide. PRD: `prds/judging-event-window-time-late-submissions.md` (2026-09-22).
- **Shortlist without the AI judge.** The View submissions table now carries the same **Shortlist for human judges** status bar as AI results (starred count, current Judge queue mode, what judges see below the cut) with links to the Judge queue setting and, only when the AI judge is on, to Shortlist top N. Starring rows there was already the manual shortlist tool; the bar makes it findable with the AI judge off and is also the way to add a submission the AI ranked outside the top N. Settings, the How to judge page, and its Markdown export stop mentioning AI ranks when the AI judge is off; admin docs Shortlist section gains a "Without the AI judge" path and spells out hand adding after Shortlist top N (2026-09-22).
- **Shortlist below the cut.** Shortlist mode gets a second option. A new **Show submissions below the cut to judges** switch under Judge queue in Settings (off by default, shortlist mode only) keeps the rest of the field visible in the judging interface as read only context. Judges get a **Shortlist / Below the cut / All** filter with counts and a "N below the cut, read only" pill; below-cut rows are grayed out in search with a Below the cut chip, open with a banner ("Not in this judging round. The AI judge ranked it #12 of 40 (7.2/10).") and the AI review card expanded, and show no score buttons, Skip, Mark Complete, or Judged & Next. Notes stay open so a judge can ask for a second look. `submitScore`, `updateSubmissionStatus`, and `markJudgeCompleted` reject below-cut rows server side; `getSubmissionStatusForJudge` returns `canJudge: false, belowCut: true`; agent queues, progress bars, and results are unchanged. An **AI #rank/total score** badge appears next to titles (always for below-cut rows, for shortlisted rows only when Show AI review to judges is on) and a **Hide AI scores** toolbar button hides badges and the card per judge and group in localStorage. AI results gains **5 / 10 / 20** presets beside Shortlist top N, a Below the cut status line, and confirm and success copy that says what judges will see of the rest. Ranking moved to `convex/lib/aiRank.ts` (`computeWeightedScore`, `compareAiResults`, `rankCompletedAiResults`) so admin results, the AI review card (`#12 of 40` in the headline), and judge badges rank identically. The How to judge page, its Markdown export, and the admin docs (Shortlist rounds, Judge queue, a new Below the cut subsection, Judge flow, Results, Shortlist top N) explain the behavior. PRD: `prds/judging-shortlist-below-the-cut.md` (2026-09-22).
- **How to judge page.** Every judging group now has a public one sheet for judges at `/judging/{slug}/howtojudge`, no login and never a passcode. It reads live group data so the judging link, results and AI results links (only when shareable), tag pages, criteria and descriptions, scoring scale with rating anchors, deadline in the viewer's time zone, and queue size are always current. Notion style layout with a TL;DR card up top (open the link, enter the access code, enter your lowercase first name and reuse it, score every criterion then Mark Complete or Judged & Next, finish by the deadline), a sticky section nav (left rail on desktop, tabs on mobile), generic screenshots of the access code screen, name form, judging interface, status buttons, scoring row, progress card, notes thread, and AI review card, a troubleshooting table, and a links card at the top and bottom. Groups with a passcode show a `passcodegoeshere` chip with "your organizer sends the real code by Slack or email"; public groups say to enter a first name. The AI review section appears only when the AI judge is on and explains what the AI reads, that it is advisory on its own 1 to 10 rubric, and how judges see it. Paused groups render with a "Judging is paused" banner. A new **How to judge** section in the admin group workspace turns the page on or off, copies the public link, edits organizer fields (access code note, judging deadline, contact, private repo notes, judge assignments, extra notes in Markdown, extra links, show results link), and has **Copy Markdown** and **Download .md** so the whole guide lands in a Google Doc in one paste. The Links ledger lists the page. PRD: `prds/how-to-judge-page.md` (2026-09-22).
- **Shortlist for human judges.** Run the AI judge first, then hand only the top N to people. AI results gets a Shortlist for human judges bar with a number input and **Shortlist top N** (confirm before replacing the set; ties at position N are all kept so sort order never cuts a team; hidden or archived stories are skipped) plus a Shortlisted badge and star toggle on every result row. The View submissions table gets a leading Shortlist column with a star per row and a Shortlist only filter. A new **Judge queue** setting in Settings switches between All submissions and Shortlist only, shows the live shortlist count, and warns in red when shortlist mode is on with nothing starred. In shortlist mode human judges, agent judges (`submissions.json`), progress bars, and results completion percentages all use the shortlist; AI runs and AI results still cover every submission so the shortlist can be picked from them. Every change is logged as `judging.shortlistChanged` (2026-09-22).
- **Show AI review to judges.** Per group toggle in the AI judge section (off by default). When on, each submission in the judging interface gets a collapsed AI review card (`AI review 7.8/10`, marked advisory) that expands to the per criterion score and reasoning, the overall note, model, and live app status. Only completed reviews appear, so a pending or failed run never shows judges an error; harness signals, git facts, and discrepancies stay admin only. The card appears and disappears live when the toggle changes, and the How to judge page updates its AI section to match (2026-09-22).
- Admin docs: new How to judge page section; Judging groups gains the How to judge workspace row, Links ledger bullet, and a Judge queue setting note; Submissions gains Shortlist and Submission deadline countdown subsections; Judge flow covers shortlist queues and the AI review card; AI judge gains Show AI review to judges, Sponsor stack detection, and the Shortlist top N step; Results notes the shortlist denominator (2026-09-22).
- **Social proof for judging.** Every submission's LinkedIn and X or Bluesky link is now snapshotted once and shared by the AI judge and every human judge, so everyone scores the same numbers captured at the same moment. X posts are read through Firecrawl's X engine (post text, author, date, likes, retweets) with a text only oEmbed fallback; Bluesky posts through Bluesky's public API (likes, reposts, replies); LinkedIn is recorded as unverified because it answers even nonexistent posts with a login wall; profile URLs are labeled as profiles, not launches. Views are never collected because only the paid X API has them. The AI prompt gains a `SOCIAL PROOF (verified by direct fetch)` section and a fixed rule that it may never invent or estimate engagement, must give LinkedIn at most partial credit as unverified, and must not treat a profile link as a launch; `sourcesUsed.socialProof` shows as a `social` chip in AI Results. A **Social proof** preset criterion joins Components check and Frontend checker in Rubric weights. Human judges see a **Social proof** card under Project Links with a Post or Profile badge, author and date, the post text, like, repost, and reply pills, the capture time, and plain language notes about what could not be read; the plain links stay as before and the card appears only once a snapshot exists. A **Social proof snapshots** card in the AI judge section has a **Refresh social proof** button that re captures every submission in the group at once, for use when submissions close. PRD: `prds/social-proof-judging.md` (2026-09-21).
- The AI judge's chat model is now picked at run time from an optional `AI_JUDGE_MODEL` Convex env var (any id from the gateway model list, `provider/model` form), with **Claude Fable 5** (`anthropic/claude-fable-5`) as the fallback. Switching models is a dashboard edit, no redeploy. Fable reads the live app screenshot, writes the per criterion reasoning, and returns the same JSON the judge already parses (2026-09-21).
- **Second opinion (Jev)**: a per group toggle in the Custom AI criteria card. When on, after Fable finishes, Jev (`typesafe/jev-1.13`, TypeSafe's decisions model on the gateway's `/alpha/decisions` endpoint) scores the same rubric from a text only copy of the evidence and returns a probability distribution over five anchored levels per criterion, mapped to 1 to 10 with a confidence percentage. Stored as `secondOpinion` on the result and shown as a `Jev 6.4 (89%)` chip next to each Fable score; gaps of 2.5 or more are flagged for human review. Advisory only: it never changes Fable's scores, the total, or the ranking, and if Jev fails the review still completes. Jev cannot take images and has a 32k token window, so the repo section is truncated for it and the result notes when that happened (2026-09-21).
- **Use Jev for the verdict** toggle in the Spam check Automation card (off by default). When on, Jev answers one choice question (spam, suspicious, clean) plus five boolean flags (dead or placeholder URL, empty or missing repo, gibberish or template description, promotional or unrelated content, links pointing elsewhere) that become the reasons list. The confidence shown and gated on by agent auto mark is the probability Jev assigned to the chosen verdict instead of a number the chat model wrote into JSON. The admin editable spam prompt still steers the verdict, and the chat model then heuristic fallbacks are unchanged (2026-09-21).
- Admin docs: AI models section covers `AI_JUDGE_MODEL`, the Fable fallback, the Jev second opinion, and the Jev spam classifier (2026-09-21).
- The AI judge now records the **sponsor stack** for every reviewed repo: whether AgentMail, Firecrawl, and OpenAI are integrated and how (Convex component used in code, npm SDK dependency or import, API key env var referenced in `convex/` source, direct HTTP call to the sponsor API, or an OpenAI model routed through the Convex AI gateway). It also names every **model provider** referenced (OpenAI, Anthropic, Google, Mistral, Groq, xAI, OpenRouter, Meta, DeepSeek) from SDK deps, API key env vars, and model ids, and the model id matcher now recognizes `gemini-`, `grok-`, `mistral-`, `llama-`, and `deepseek-` families it used to drop. Both land in the prompt as a `SPONSOR STACK EVIDENCE` section plus a providers line in `AI MODEL EVIDENCE`, with a fixed rule that the model must name what they show and never move a rubric score for sponsor usage; the sponsor stack stays a human criterion. Stored on `aiJudgeResults` as `sponsorStack` and `modelProvidersDetected`, shown as chips in the admin results cards, compare view, and the public results page, added to the per-submission brief, the Convex team recap, and the hackathon report table, and rolled up in a new Stats card. PRD: `prds/ai-judge-sponsor-stack-detection.md` (2026-09-21).
- The AI judge can now score the same **human judging criteria** organizers write in the Criteria editor. A "Human judging criteria" block at the top of Custom AI criteria has an **In AI rubric** toggle; when on, every human question joins the AI rubric under a stable `human-<criteriaId>` key, shows up tagged `human` in Rubric weights with its own AI weight and On/Off pill, and appears in the `{{rubric}}` preview on the prompt card. The AI reads the question and description and scores 1 to 10 from the live app, screenshot, video transcript, and repo; the prompt tells it the human scale (1 to 5 or 1 to 10) so intent matches. Human weights and the human scale are untouched. Criteria stay editable only in the Criteria section, edits apply on the next run, saved results keep their labels, and deleting a criterion prunes its AI weight and toggle automatically (2026-09-21).
- The AI judge now **sees the live app**: the existing Firecrawl scrape also captures a screenshot of the rendered first screen and it is attached to Claude Sonnet 4.5 through the Convex AI gateway as an image. Fixed prompt rules limit it to UI and frontend judgments (never Convex facts) and say a missing screenshot never lowers a score. The image rides on the first attempt only; the retry is text only, so a bad image can never fail a review. `sourcesUsed.screenshot` records whether it was used (2026-09-21).
- `judgeScores.by_criteriaId` index so the criterion delete guard is an indexed lookup instead of a table scan (2026-09-21).
- Admin docs: Human criteria in the AI rubric and Live app screenshot sections, the stale Anthropic, OpenAI, and OpenRouter key tables replaced with the Convex AI gateway note (no provider keys are read anymore), a Criteria and weights line on separate human and AI weights, and an "AI judge vs agent judges" paragraph explaining that agent keys are for outside agents posting to the HTTP API and are not needed for the built in AI judge (2026-09-21).
- Judging group custom submission pages can show a live countdown to the submission deadline. Admins turn it on in the Custom Submission Page section, pick the deadline with a local time picker (stored as an absolute timestamp so every visitor sees the same instant in their own zone, e.g. "Tue, Sep 22, 12:00 PM PDT" for a PT admin and the matching local time elsewhere), choose Large (stacked days, hours, minutes, seconds) or Compact (one line), place it at the top of the page or above the Submit Your App form, and override the heading (default "Submissions close in"). A live preview in the admin card renders exactly what visitors see. The timer is display-only: it reads "Deadline passed" after the deadline and never blocks submissions, so the existing Open/Closed switch remains the gate. Screen readers get a summary label; the ticking digits are aria-hidden. Built as a small in-house component, no new dependency (2026-09-21).
- `.env.example` now exists (the README told you to copy it, but the file was missing). It lists the frontend `VITE_*` keys, every Convex deployment env var the code reads including the `FIRECRAWL_API_KEY` and `CONTEXT_DEV_API_KEY` that `convex.config.ts` requires at deploy time, and the Clerk `convex` JWT template with the claims `ensureUser` depends on. Without that template Clerk issues no token and every authenticated call fails with "User not authenticated" while the browser still looks signed in. From mooler-z's [PR #18](https://github.com/waynesutton/vibeapps/pull/18); the setup and correctness commits were cherry-picked with attribution, the UI redesign is parked for a separate design review (2026-09-21).
- `npm run typecheck` (app, `tsc -b`) and `npm run typecheck:convex` (backend) scripts, plus `lint:fix` and `dev:backend`. There was no way to typecheck the frontend before; `vite build` strips types and never reported the errors. `@types/node` added so `vite.config.ts` and `NodeJS.Timeout` resolve (PR #18, 2026-09-21).
- `convex/_generated/ai/guidelines.md` and the generated `convex-*` agent skills, installed with `npx convex ai-files install`. `AGENTS.md` and `CLAUDE.md` already told agents to read the guidelines file, which did not exist in the repo (2026-09-21).
- `convex/lib/reactionEmojis.ts` exports the DM reaction emoji list without importing anything from `convex/values`, so the frontend can build a picker from it. `dmReactions.ts` type-checks its validator against that list so the two cannot drift (PR #18, 2026-09-21).
- Judging group submissions can now be downloaded as CSV, JSON, or Markdown from a **Download** dropdown in the View submissions toolbar, next to the roster it exports. JSON wraps the same export rows with group metadata and an export timestamp. Markdown arrives as a ZIP with one numbered file per submission plus a `README.md` index; each file carries YAML frontmatter (`title`, `slug`, `url`, `page`, `tags`, `votes`, `teamName`, `submitter`) so agents and static site tools can read it without parsing prose. The long description is written as the author's original Markdown. Only `judging.results` holders see the control, matching the server gate. Thanks to cuevaio for [PR #17](https://github.com/waynesutton/vibeapps/pull/17) (2026-09-03, merged 2026-09-21).
...
- Judging interface: the Video Demo now sits inside the submission card directly above Detailed Description, so judges watch the demo before reading the long writeup. It was a separate card below the description. Same embed logic (YouTube, Vimeo, Loom, Google Drive, direct files, link fallback) and the same open in new tab arrow (2026-09-22).
- `convex/lib/llm.ts` no longer hardcodes the chat model: `DEFAULT_LLM_MODEL` is now `FALLBACK_LLM_MODEL` behind `resolveLlmModel()`, which reads the typed `AI_JUDGE_MODEL` env declared in `convex.config.ts`. `getSpamPromptInternal` became `getSpamAnalysisConfigInternal` and returns the prompt plus the Jev classifier flag in one call. `ai` and `@convex-dev/ai-sdk-provider` bumped for `experimental_evaluate` and `convexGateway.evaluationModel` (2026-09-21).
- The judging group CSV export moved out of the manage-only Submissions card and into the View submissions Download dropdown. The CSV itself is hardened: cells that start with `=`, `+`, `-`, or `@` get a leading apostrophe so spreadsheets cannot run them as formulas, multiline cells are quoted, lines end in CRLF, and the file opens with a UTF-8 BOM so Excel reads accents correctly. The export builders now live in `src/lib/submissionDownloads.ts` and JSZip loads on demand only when a Markdown archive is requested, so the main bundle is unchanged ([PR #17](https://github.com/waynesutton/vibeapps/pull/17), 2026-09-21).
- Submission download feedback uses the same `sonner` toasts as the rest of the group workspace instead of an inline status line (2026-09-21).
- Package renamed from `vite-react-typescript-starter` to `vibeapps`. `caniuse-lite` dropped as a direct dependency (browserslist pulls it in). Vite no longer excludes `lucide-react` from dependency pre-bundling in dev. `.gitignore` covers `.env` and `*.tsbuildinfo`. 84 unused imports and variables cleared across `convex/` and `src/` so the new typecheck baseline is zero errors (PR #18, 2026-09-21).

### Fixed

- AI ranks now count one population everywhere: stories that are current members of the group and still valid for judging. `enrichResults` (admin AI results, public and validated results) and `getAiReviewForJudge` counted stale `aiJudgeResults` rows left behind when a story was removed from a group before `removeSubmission` started deleting them, so admin showed "4 of 6 flagged" and the judge card said `#1 of 6` while the judge badge said `of 4`. Both now skip rows whose story has no `judgingGroupSubmissions` row, matching `getGroupSubmissions` and `shortlistTopByAiScore`. Below-cut rows in the judge search dropdown also no longer show an empty `(by )` label (2026-09-22).
- The AgentMail Convex component (`@agentmail/convex`) was reported as both "used in code: agentmail" and "installed but not referenced: @agentmail/convex" in the same prompt, and a hackathon.md that listed `components: agentmail` raised a false discrepancy. The package is not under `@convex-dev/*` and its import ends in `/convex`, so neither name normalizer produced `agentmail`. Added the map entry, and `@scope/convex` imports now resolve to the scope name so future sponsor components work without a map edit (2026-09-21).
- The legacy `EditJudgingGroupModal` saved only the six built in rubric weights, so a save there wiped custom criteria weights (and would have wiped mirrored human weights). It now carries the group's stored non built in weights through untouched. The preset Add buttons in Rubric weights also counted human rows toward the custom criteria limit; they now count real custom criteria (2026-09-21).
- Admin docs claimed human judging criteria "can have weights that multiply into the weighted totals." No human ranking query reads `judgingCriteria.weight`; human results are plain totals and averages of the 1 to 5 or 1 to 10 scores. The Criteria and AI judge doc sections now say human scoring is unweighted and that weights apply to the AI judge only (2026-09-21).
- Fenced code blocks in Markdown rendered as the literal text `[object Object], ,[object Object]` with highlighting destroyed, and inline code became a `<pre>` nested inside a `<p>`. react-markdown v9 removed the `inline` prop that `Markdown.tsx` branched on, so the branch was dead. The component now styles the `pre` wrapper react-markdown already emits and leaves inline spans alone (PR #18, 2026-09-21).
- `src/types/index.ts` imported `../convex/_generated/dataModel`, which resolves to a nonexistent `src/convex/`, so `Doc` and `Id` were silently `any` and type errors across the app went unreported. Path fixed; the 95 errors that surfaced are resolved, including 7 real ones in `ResendForm`, `FormBuilder` (now checked against the exported `FormFieldType` from `convex/forms.ts`), `InboxPage`, and `UserProfilePage` (PR #18, 2026-09-21).

- `npx convex dev` and `npx convex deploy` failed with `Could not resolve "zod/v4"` after the lockfile was regenerated under `.npmrc` `legacy-peer-deps=true`: npm stopped auto-installing `zod`, which the `ai` SDK and `@ai-sdk/*` packages only declare as a peer, so the Convex bundler could not find it. `zod` is now a direct dependency (2026-09-21).
- Upgraded `@convex-dev/resend` 0.2.6 to 0.2.7 and `@convex-dev/workpool` 0.3.2 to 0.4.10. The new Workpool uses a snapshot query in its main loop so enqueue and complete no longer take read dependencies on `pendingStart` and `pendingCompletion`, which is where the `resend/emailWorkpool` and `resend/callbackWorkpool` OCC retries in Insights were coming from. Prod picks this up on the next `npx convex deploy` (2026-08-30).
...
- Netlify builds failed at `npm install` with `ERESOLVE`: `@waynesutton/agent-ready` declares a `@convex-dev/workpool ^0.3.0` peer while `@convex-dev/resend` 0.2.7 and the app need `^0.4.10`, and no published agent-ready version accepts 0.4 yet. A root `.npmrc` with `legacy-peer-deps=true` lets npm install the same tree the workstation and the prod Convex deployment already run (workpool 0.4.10 shared by both components). Every peer agent-ready needs is a direct dependency, so nothing is skipped. Temporary until agent-ready widens its peer range (2026-09-22).
- Editing your own hackathon submission from the story page failed with a redacted `Server Error` (GitHub issue #19). Public story pages strip team member emails for privacy, the edit form sent those email-less members back, and `updateOwnStory` requires an email per member. `updateOwnStory` and `updateStoryAdmin` now accept members without an email and keep the stored address for that position, so an untouched member is never wiped. The edit form leaves the team section out of the save unless you changed it, and a blank email keeps the saved one. Tag limits on that form now come from admin settings (visible tags only, new tag name length checked) instead of a hardcoded 10, and a submission that already exceeds the current limit says how many to remove before it can be saved (2026-09-22).
- Submitting the same project twice to one hackathon event showed `[Request ID: ...] Server Error Called by client` with no explanation. The event submit page now checks the App Website Link as you type and, when that URL is already in the group, explains it under the field and blocks submit with a pointer to Edit on the existing submission (2026-09-22).
- Submit forms gated only on Clerk sign in, so a click during the short window before Convex verified the session hit `User not authenticated` on the server and surfaced as `Server Error`. Both the story submit form and the event submit page now wait for Convex auth, show "Connecting your account..." on the button while it settles, and explain instead of sending a request that would fail (2026-09-22).
- Every user facing check in story submit, anonymous submit, owner edit, and admin edit (image count, rate limit, duplicate event URL, tag count and tag name length, at least one tag, owner check, slug clash, account not synced, banned account) now throws `ConvexError`, so production shows the real sentence instead of `Server Error`. The submit rate limit message now says 20 per day to match the check. Mutation errors on the submit and edit forms are rendered through a shared helper that shows `ConvexError` messages as written, strips the `[CONVEX M(...)] [Request ID: ...]` wrapper, and turns any remaining redacted error into a plain sentence that keeps the request id so participants can paste it in Discord. The hackathon log size cap throws `ConvexError` too (2026-09-22).
- Saving a hackathon submission from the admin Content Moderation form wiped its team member emails. `listAllStoriesAdmin` strips member emails as PII, and the form sent that stripped list back to `updateStoryAdmin`, which patched it verbatim. The form now only sends `teamMembers` when the admin actually edited them and omits `email` for members whose address never reached the client; `updateStoryAdmin` accepts an optional email and merges by position so a missing email keeps the stored one. The public story edit path no longer crashes on a member without an email (2026-09-21).
