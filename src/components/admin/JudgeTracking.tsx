...
export function JudgeTracking({
  ...
  const handleToggleScoreVisibility = async (
    ...
    try {
      ...
      showMessage(
        "Error",
        "Failed to update score visibility. Please try again.",
        "error",
      );
    }
  };
  ...
  const filteredJudges = judges.filter((judge) => {
    ...
    // Search in judge basic info
    const matchesJudge =
      judge.name.toLowerCase().includes(searchLower) ||
      judge.email?.toLowerCase().includes(searchLower) ||
      judge.userProfile?.username?.toLowerCase().includes(searchLower);

    if (matchesJudge) return true;

    // Search in judge scores if expanded
    if (expandedJudges.has(judge._id) && judgeScores) {
      const matchesInScores = judgeScores.some(
        (score) =>
          score.story.title.toLowerCase().includes(searchLower) ||
          score.story.slug.toLowerCase().includes(searchLower) ||
          score.criteria.question.toLowerCase().includes(searchLower) ||
          score.criteria.description?.toLowerCase().includes(searchLower) ||
          score.comments?.toLowerCase().includes(searchLower),
      );

      if (matchesInScores) return true;

      // Search in submission notes
      if (submissionNotes) {
        const matchesInNotes = submissionNotes.some(
          (note) =>
            note.content.toLowerCase().includes(searchLower) ||
            note.judgeName.toLowerCase().includes(searchLower) ||
            note.replies?.some(
              (reply) =>
                reply.content.toLowerCase().includes(searchLower) ||
                reply.judgeName.toLowerCase().includes(searchLower),
            ),
        );

        if (matchesInNotes) return true;
      }
    }
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <div className="bg-surface rounded-lg border border-hairline p-4">
            <div className="flex items-center gap-2 mb-2">
              <BarChart3 className="w-4 h-4 text-soft" />
              <span className="text-sm font-medium text-copy">
                Avg Score
              </span>
            </div>
              ...
              {filteredJudges.length > 0
                ? (
                    filteredJudges.reduce(
                      ...
                      0,
                    ) / filteredJudges.filter((j) => j.averageScore).length
                  ).toFixed(1)
                : "0.0"}
            </div>
          </div>
        ...
        >
          ...
          {judges.length === 0 ? (
            ...
            <div className="p-8 text-center text-soft">
              ...
              <button
                onClick={() => setSearchTerm("")}
                className="mt-4 text-blue-600 hover:text-blue-800 hover:underline text-sm"
              >
                Clear search
              </button>
            </div>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {judge.type === "agent" && (
                            <span
                              className="inline-flex items-center px-2 py-0.5 rounded-full text-xs border border-hairline bg-surface-alt text-soft"
                              title={
                    ...
                    <div className="flex items-center gap-6 text-sm text-soft ml-auto">
                      ...
                      <div className="text-center min-w-16">
                        <div className="font-medium text-ink flex items-center gap-1 justify-center">
                          {judge.notesCount || 0}
                          <MessageSquare className="w-3 h-3 text-soft" />
                        </div>
                                ...
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      {noteCountsData &&
                                        noteCountsData[score.story._id] > 0 && (
                                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs border border-hairline bg-surface-alt text-soft">
                                  ...
                                  <div className="flex items-center gap-1 ml-2">
                                    <button
                                      ...
                                      className="p-1 text-soft hover:text-ink hover:bg-surface-hover rounded"
              ...
              <div className="flex flex-wrap gap-1 p-4">
                {judgeDetailsData.map((judge, index) => (
                  <button
                    ...
                    aria-pressed={selectedJudgeTabIndex === index}
