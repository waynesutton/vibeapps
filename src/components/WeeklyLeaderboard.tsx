...
export function WeeklyLeaderboard() {
  const topStories = useQuery(api.stories.getWeeklyLeaderboardStories, { limit: 5 });

  if (topStories === undefined) {
    return (
      <div className="p-4 bg-white rounded-lg border border-[#D8E1EC]">Loading leaderboard...</div>
    );
  }
  ...
  if (!topStories || topStories.length === 0) {
    return (
      <div className="p-4 boohide bg-white rounded-lg border border-[#D8E1EC]">
        ...
        <p className="text-sm text-[#545454]">No apps trending this week yet.</p>
      </div>
    );
  }
      ...
      <ul className="space-y-3">
        {topStories.map((story, index) => (
          <li key={story._id} className="flex items-start gap-3">
            <span className="text-sm font-medium text-[#787672] pt-0.5">{index + 1}.</span>
